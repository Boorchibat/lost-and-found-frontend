...
const COLORS = ["Red", "Blue", "Yellow", "Green", "Black", "White", "Gray", "Orange", "Purple", "Pink", "Brown", "other"];
...
export const ModalUpdateItem = ({ open, handleClose, item }: ModalProps) => {
  ...
  const [persistedImages, setPersistedImages] = useState<ImageType[]>(item.images || []);
  ...
  return (
    <>
      <Modal open={open} onClose={handleClose}>
        <Box sx={style}>
          <h1 className="text-3xl font-extrabold text-center mb-6">Update Item</h1>
            ...
            onSubmit={async (values, { setSubmitting }) => {
              try {
                ...
                if (imageFiles.length > 0) {
                  const uploads = await Promise.all(imageFiles.map(f => uploadToCloudinary(f)));
                  newImages.push(...uploads.map(u => ({ url: u.secure_url, public_id: u.public_id })));
                }
          ...
          >
            {({ handleChange, values, isSubmitting, errors }) => (
              <Form className="flex flex-col gap-5">
                ...
                <div className="flex gap-4 items-start">
                  <div>
                    ...
                    <label className="relative block w-24 h-24 border-2 border-dashed rounded cursor-pointer overflow-hidden">
                      <Image 
                        src={mainImageFile ? URL.createObjectURL(mainImageFile) : (item.mainImage?.url || "/file.svg")} 
                        alt="Main" fill className="object-cover" 
                      />
                      <input type="file" className="hidden" onChange={handleMainFileUpload} />
                    </label>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {persistedImages.map(img => (
                        <div key={img.public_id} className="relative w-16 h-16">
                          <Image src={img.url} alt="Gallery" fill className="object-cover rounded" />
                          <XCircle 
                            onClick={() => setPersistedImages(prev => prev.filter(i => i.public_id !== img.public_id))} 
                      ...
                      {imageFiles.map((file, i) => (
                        <div key={i} className="relative w-16 h-16 border border-blue-400">
                          <Image src={URL.createObjectURL(file)} alt="New" fill className="object-cover rounded" />
                          <XCircle 
                      ...
                      <label className="flex items-center justify-center w-16 h-16 border-2 border-dashed rounded cursor-pointer">
                        ...
                        <input type="file" multiple className="hidden" onChange={handleMultipleFilesUpload} />
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {COLORS.map(c => (
                        <label key={c} className={`text-[15px] p-2 rounded cursor-pointer ${values.color.includes(c) ? 'bg-blue-600 text-white' : 'bg-gray-100'}`}>
                          <input type="checkbox" name="color" value={c} checked={values.color.includes(c)} onChange={handleChange} className="hidden" /> {c}
                        </label>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {PHYSICALS.map(p => (
                        <label key={p} className={`text-[15px] p-2 rounded cursor-pointer ${values.physical.includes(p) ? 'bg-green-600 text-white' : 'bg-gray-100'}`}>
                          <input type="checkbox" name="physical" value={p} checked={values.physical.includes(p)} onChange={handleChange} className="hidden" /> {p}
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                <Button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white h-12 rounded-xl">
                  {isSubmitting ? "Updating..." : "Update Item"}
                </Button>
                
                {Object.keys(errors).length > 0 && (
                  <p className="text-red-500 text-center text-xs">Please fill in all required fields</p>
                )}
              </Form>
            )}
          </Formik>
      ...
      <Modal open={showSuccessModal} onClose={() => setShowSuccessModal(false)}>
        <Box sx={{ ...style, maxWidth: 350, textAlign: "center" }}>
          ...
          <p className="text-gray-600 text-sm mb-4">Item details have been saved.</p>
          <Button onClick={() => setShowSuccessModal(false)} className="w-full bg-green-600 text-white py-2 rounded-lg">
            Close
          </Button>
        </Box>
      </Modal>
